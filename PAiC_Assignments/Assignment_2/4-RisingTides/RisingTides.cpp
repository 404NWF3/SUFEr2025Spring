...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Vector<int> des_row = {0, 0, 1, -1};
    Vector<int> des_col = {-1, 1, 0, 0};

    int length_row = terrain.numRows(), length_col = terrain.numCols();
    Grid<bool> res(length_row, length_col, false);

    Queue<GridLocation> floodedRegions;
    //把sources中的点都加入floodedRegions中
    for (const GridLocation& src : sources){
        if (src.row >= 0 && src.row < length_row && src.col >= 0 && src.col < length_col) {
            if (terrain[src.row][src.col] <= height) {
                res[src.row][src.col] = true;
                floodedRegions.enqueue(src);
            }
        }
    }
    ...
    while(!floodedRegions.isEmpty()){
        //已经在队列中的点已经被淹掉了
        GridLocation curLocation = floodedRegions.dequeue();
        int curCol = curLocation.col, curRow = curLocation.row;

        //遍历每一个方向
        for (int i = 0; i < 4; i++){
            int x = des_row[i], y = des_col[i];
            // 判断每一个方向的点是否被淹
            // 条件是terrain[curRow+x][curCol+y] <= height
            int new_row = curRow+x, new_col = curCol+y;

            //边界检验
            if (new_row < 0 || new_col < 0 || new_row >= length_row || new_col >= length_col) {
                continue;
            }

            //被淹了的跳过
            if (res[new_row][new_col]) continue;

            if (terrain[new_row][new_col] <= height){
                GridLocation temp(new_row, new_col);
                res[new_row][new_col] = true;
                floodedRegions.enqueue(temp);
            }
